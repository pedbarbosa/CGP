...
class Type_GenericIO extends Type_Base {
	...
	public function rrd_gen_graph() {
		...
		# GenericIO expects exactly 2 sources (read + write). If we have fewer,
		# fall back to a simple single-line graph to avoid broken rrdtool commands.
		$has_two_sources = count($sources) >= 2;
		...
		if ($this->scale) {
			...
			foreach ($this->tinstances as $tinstance) {
				foreach ($this->data_sources as $ds) {
					...
					if ($this->percentile && $this->negative_io && $has_two_sources && $i % 2 == 1)
		...
		foreach($sources as $source) {
			$rrdgraph[] = sprintf('AREA:avg_%s%s#%s', crc32hex($source), ($has_two_sources && $i % 2 == 1) ? '_neg' : '', $this->get_faded_color($this->colors[$source]));
			$i++;
		}

		if ($has_two_sources && !$this->negative_io) {
		...
		foreach($sources as $source) {
			...
			$rrdgraph[] = sprintf('LINE1:avg_%s%s#%s:%s', crc32hex($source), ($has_two_sources && $i % 2 == 1) ? '_neg' : '', $this->colors[$source], $this->rrd_escape($legend));
		...
		if ($this->percentile) {
			...
			foreach($sources as $source) {
				$legend = empty($this->legend[$source]) ? $source : $this->legend[$source];
				$rrdgraph[] = sprintf('LINE:pct_%s%s#%s:%sth Percentile %s', crc32hex($source), ($has_two_sources && $i % 2 == 1 && $this->negative_io) ? '_neg' : '', $this->get_faded_color($this->colors[$source], '000000', 0.6), $this->percentile, $this->rrd_escape($legend));
